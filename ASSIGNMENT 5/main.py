...
@app.get('/order/search')                                       # Q.4
def search_order(
    cust_name : str = Query(...)
):
    result = [k for k in orders if cust_name.lower() in k['customer_name'].lower()]
    if not result:
        return {'message': f'No order found for: {cust_name}'}
    return {
...
@app.get('/products/sort_category')                             # Q.5
def SortByCategory():
    return{
    'products' : sorted(products, key=lambda p: (p['category'], p['price']))
    }

@app.get('/products/browse')                                    # Q.6
def browse_products(
