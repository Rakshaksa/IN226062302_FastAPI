...
# for assignment go to 210 line of code
...
#     return {'orders': orders, 'total_orders': len(orders)}
                                            ...
                                            # ASSIGNMENT 3
@app.post('/products')                          # Q.1
def add_new_product(new_product : NewProduct, response : Response):
    exist_names = [p['name'].lower() for p in products]
    if new_product.name.lower() in exist_names:
        response.status_code = status.HTTP_400_BAD_REQUEST
        return {'error':'product with this name is already exist'}
...
@app.put('/products/id/{product_id}')                  # Q.2
...
@app.delete('/products/{product_id}')                   # Q.3
def delete_specific_product(product_id :int, response: Response):
    product = find_product(product_id)
    if not product:
        response.status_code = status.HTTP_404_NOT_FOUND
        return {'error':'product not found'}
...
@app.put('/products/discount')                  #bonus
def bulk_discount(
    category : str = Query(...),
    discnt_percent : int = Query(...,gt = 1, le = 99)
):
    updated = []
    for p in products:
        if p["category"] == category:
            p["price"] = int(p["price"] * (1 - discnt_percent / 100))
            updated.append(p)
    ...
    return {
        "message": f"{discnt_percent}% discount applied to {category}",
